...
export default function OpenSourcePage() {
  return (
    <div className="page">
      <PageHead title="Open source">Open-source hardware and firmware let you learn from industrial code and build public evidence. Start by using the tools, then contribute in small, careful steps.</PageHead>
      ...
      <div className="grid2">{OPEN_SOURCE.map((o) => (
        <section key={o.id} id={o.id} className="panel anchor">
          ...
          <dl className="qa small"><dt>Why it matters</dt><dd>{o.why}</dd><dt>Use it as a beginner</dt><dd>{o.beginnerUse}</dd><dt>How to contribute</dt><dd>{o.contribute}</dd><dt>Portfolio value</dt><dd>{o.portfolio}</dd>{o.study && <><dt>What to read</dt><dd>{o.study}</dd></>}{o.exercise && <><dt>Exercise</dt><dd>{o.exercise}</dd></>}{o.license && <><dt>Licence</dt><dd>{o.license}</dd></>}</dl>
        </section>))}
      </div>
