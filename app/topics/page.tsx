...
import { ALL_PHASES, phaseLabel } from "@/content/phases";
...
export default function TopicsPage() {
  ...
  const list = TOPICS.filter((t) =>
    (phase === "all" || t.phase === phase) && (prog === "all" || (prog === "embedded") === t.phase.startsWith("e")) && (prio === "all" || t.priority === prio) &&
    ...
    <div className="page">
      ...
      <div className="panel">
        <div className="grid3">
          ...
          <div className="field"><label htmlFor="tprog">Program</label><select id="tprog" value={prog} onChange={(e) => setProg(e.target.value)}><option value="all">VLSI and Embedded</option><option value="vlsi">VLSI road</option><option value="embedded">Embedded road</option></select></div>
          <div className="field"><label htmlFor="tp">Phase</label><select id="tp" value={phase} onChange={(e) => setPhase(e.target.value)}><option value="all">All phases</option>{ALL_PHASES.map((p) => <option key={p.id} value={p.id}>{phaseLabel(p)}: {p.short}</option>)}</select></div>
